...
# We are doing it for Pydantic model here
class ReviewAnalysis(BaseModel): 
  
  # For tyedict we used annotations to add descriptions
  # e.g., Annotated[list[str], "List of key themes mentioned in the review"]
  
  # for Pydantic we can use Field from pydantic to add descriptions
  # e.g., key_themes: list[str]= Field(..., description="List of key themes mentioned in the review")  
  
  key_themes: list[str]= Field(..., description="List of key themes mentioned in the review") # e.g., ["Camera", "Battery Life", "Design"]   # List of key themes mentioned in the review
  
  summary: list[str]= Field(..., description="Concise summary of the review") # e.g., "The iPhone 15 offers significant camera improvements and USB-C support, but design changes are minimal."  # Concise summary of the review
  sentiment: list[str]= Field(..., description="Overall sentiment of the review") # e.g., "Positive"  # Overall sentiment of the review
  recommendation: list[str]= Field(..., description="Whether the reviewer recommends the product") # e.g., "Yes"  # Whether the reviewer recommends the product
  pros: list[str]= Field(..., description="List of pros mentioned in the review") # e.g., "Fast shipping, Good quality"
  cons: list[str]= Field(..., description="List of cons mentioned in the review") # e.g., "Poor customer service, Expensive"
